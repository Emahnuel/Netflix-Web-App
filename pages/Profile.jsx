import React from "react";
import Navbar from "../components/Navbar";
import { signOut } from "firebase/auth";
import { auth } from "../src/firebase";
import { useNavigate } from "react-router-dom";
import PlanScreen from "../components/PlanScreen";

const Profile = () => {

const navigate = useNavigate();
const handleSignOut = async () => {
    try {
      await signOut(auth);
      console.log("User signed out successfully");
      navigate("/");
      // Optionally redirect the user or clear any local state here
    } catch (error) {
      console.error("Error signing out:", error.message);
    }
  };
  return (
    <>
      <Navbar />
      <header className="flex flex-col justify-center items-center bg-black h-screen">
        <div className="bg-black p-20 -[45rem] text-white">
          <h1 className="text-white text-6xl font-bold">Edit Profile</h1>

          <div className="flex flex-row pt-[2rem]">
            <img
              className="h-[7rem] w-[7rem] mr-[2rem]"
              src="data:image/png;base64,"
            />
            <div>
              <h1 className="flex items-center pl-[2rem] h-[3rem] bg-gray-500">
                
              </h1>
              <h1 className="pt-[2rem] text-3xl font-bold">Plans (Current Plan:)</h1>
              <h1 className="pt-[2rem] text-2xl">Renewal date: </h1>
              
              <PlanScreen/>
              <button onClick={handleSignOut} className="flex justify-center items-center bg-red-600 mt-[4rem] text-2xl px-8 py-2"> Sign out</button>
            </div>
          </div>
        </div>
      </header>
    </>
  );
};

export default Profile;
